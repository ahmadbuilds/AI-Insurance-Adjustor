...
from domain.intent import ClassificationStatus, SameVehicleStatus
...
class SameVehicleAgentState(BaseModel):
    claim_id: str = Field(description="The claim being processed")
    user_id: str = Field(description="The user who filed the claim")
    vehicle_images: list[ImageWithUrl] = Field(default_factory=list, description="Images that contain vehicles (is_vehical=True)")
