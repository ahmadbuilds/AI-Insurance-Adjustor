from domain.ports import VectorStorePort, PolicyRetrievalPort
from infrastructure.vector_store.chroma_client import create_chroma_instance

class ChromaVectorStore(VectorStorePort, PolicyRetrievalPort):
    def __init__(self):
    ...
    def query_policy(self, query: str, k: int = 5) -> list[dict]:
        """
        Perform a semantic similarity search against the policy document collection.
        Args:
            query: The search query string.
            k: Number of top results to return.
        Returns:
            list[dict]: List of dicts with 'content' and 'metadata' keys.
        """
        try:
